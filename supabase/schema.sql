...
begin
  ...
  update public.daily_recon_jobs set status='needs_review',updated_at=now()
    where business_date between p_from and p_to and error_count>0;
  update public.daily_recon_jobs set status='queued',updated_at=now()
    where business_date between p_from and p_to and status='ready' and error_count=0;
  update public.daily_recon_jobs set status='needs_review',updated_at=now()
  ...
  insert into public.recon_notifications(dedupe_key,level,title,detail,business_date,company,job_id)
    ...
    from public.daily_recon_jobs j where j.status='needs_review' and j.error_count=0 on conflict(dedupe_key) do nothing;
  insert into public.recon_notifications(dedupe_key,level,title,detail,business_date,company,job_id)
...
create or replace function public.record_source_file_parse_results(p_job_id uuid,p_results jsonb)
returns jsonb language plpgsql security definer set search_path=public as $$
declare
  v_job public.daily_recon_jobs%rowtype; v_result jsonb; v_file_id uuid; v_error text;
  v_row_count integer; v_updated integer:=0; v_errors integer:=0; v_error_files text[]:='{}';
begin
  ...
  for v_result in select value from jsonb_array_elements(coalesce(p_results,'[]'::jsonb)) loop
    ...
    update public.source_files f set parsed=v_error is null,
      parsed_at=case when v_error is null then now() else null end,row_count=v_row_count,parse_error=v_error
      where f.id=v_file_id and exists(select 1 from public.mail_batches b where b.id=f.batch_id
        and b.business_date=v_job.business_date and upper(coalesce(f.company,b.company,''))=upper(v_job.company));
    if found then
      v_updated:=v_updated+1;
      if v_error is not null then v_errors:=v_errors+1;
        if cardinality(v_error_files)<8 then v_error_files:=array_append(v_error_files,coalesce(v_result->>'file_name',v_file_id::text)); end if;
      end if;
    end if;
  end loop;
  if v_updated=0 then raise exception 'no source files were updated for job %',p_job_id; end if;
  if v_errors>0 then
    update public.daily_recon_jobs set status='needs_review',parsed_count=greatest(v_updated-v_errors,0),
      error_count=v_errors,claimed_at=null,claimed_by=null,
      last_error=left('Quality Gate: อ่านไฟล์ไม่ผ่าน '||v_errors||' ไฟล์ — '||array_to_string(v_error_files,', '),2000),updated_at=now()
      where id=p_job_id;
    insert into public.recon_notifications(dedupe_key,level,title,detail,business_date,company,job_id)
      values('parse-quality:'||p_job_id||':'||md5(coalesce(p_results::text,'')),'error','อ่านไฟล์ไม่ผ่าน Quality Gate',
        'พบ '||v_errors||' ไฟล์: '||array_to_string(v_error_files,', '),v_job.business_date,v_job.company,p_job_id)
      on conflict(dedupe_key) do nothing;
  else
    update public.daily_recon_jobs set parsed_count=v_updated,error_count=0,last_error=null,updated_at=now() where id=p_job_id;
  end if;
