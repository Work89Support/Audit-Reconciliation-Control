...
const Manual = (() => {
  ...
  const SECTIONS = [
    ...
    },
    {
      id: "fx",
      group: "ความเสียหาย",
      title: "ค่าเงิน USDT ประจำวัน",
      blocks: [
        { t: "img", src: IMG("31-fx"), cap: "หน้าบันทึกอัตราแลกเปลี่ยนประจำวัน พร้อมประวัติย้อนหลัง" },
        { t: "p", text: "ระบบมีทั้งเงินบาทและ USDT (เช่น ถอนจาก Azpay เป็น USDT เข้าหลัก B10) ยอดที่เป็น USDT ต้องอ้างอิงอัตราของ “วันนั้น” เสมอ ไม่ใช่อัตราวันที่มาเปิดดู จึงต้องบันทึกอัตราไว้ทุกวัน" },
        { t: "h", text: "ขั้นตอนบันทึกประจำวัน" },
        {
          t: "steps",
          items: [
            "เปิดเมนู “ค่าเงิน USDT”",
            "กดปุ่ม “ดึงเรตอ้างอิงจากเว็บ” เพื่อดูราคาตลาด (Bitkub / ExchangeRate API / CoinGecko)",
            "กรอกอัตราที่บริษัทใช้ปิดยอดจริงลงช่อง “1 USDT เท่ากับ” — จะกด “ใช้ค่านี้” จากเรตตลาดแล้วแก้ก็ได้",
            "ใส่หมายเหตุว่าเรตนี้มาจากไหน เช่น เรตที่ใช้ปิดยอดกับหลัก B10",
            "กดบันทึก ระบบจะลง Audit Log ทันทีว่าใครบันทึก เวลาไหน",
          ],
        },
        { t: "warn", text: "เรตจากเว็บมีไว้เทียบเท่านั้น ระบบไม่เอามาใช้เอง เพราะอัตราที่ถูกต้องคืออัตราที่บริษัทใช้ปิดยอดจริง ถ้าค่าที่กรอกห่างจากตลาดเกิน 2% ระบบจะขึ้นเตือนสีแดงให้ตรวจซ้ำ" },
        { t: "h", text: "การแก้ไขย้อนหลัง" },
        { t: "p", text: "แก้อัตราของวันที่ผ่านมาได้ แต่ระบบจะไม่ทับของเดิมทิ้ง — เก็บค่าเดิมไว้ในคอลัมน์ “แก้ไข” และบันทึกลง Audit Log ว่าเปลี่ยนจากเท่าไรเป็นเท่าไร ใครเป็นคนแก้ ใช้ตอบการเงินได้เมื่อถูกตรวจสอบ" },
        {
          t: "table",
          head: ["จุดที่ใช้อัตรา", "ใช้ยังไง"],
          rows: [
            ["ชิป USDT บนแถบบน", "บอกทันทีว่าวันที่ตรวจอยู่ลงเรตแล้วหรือยัง สีแดง = ยังไม่ลง สีเหลือง = ใช้เรตวันก่อนหน้าอยู่"],
            ["ทะเบียนความเสียหาย", "แสดงยอดทั้งบาทและ USDT ถ้าเคสไหนบันทึกเป็น USDT จะแปลงเป็นบาทด้วยเรตของวันนั้น"],
            ["ใบขอให้ชี้แจง / เอกสารชี้แจง", "พิมพ์บรรทัดอัตราที่ใช้ลงในเอกสาร PDF ด้วย"],
            ["Export Excel", "มีชีต “อัตราแลกเปลี่ยน” และคอลัมน์สกุล/อัตรา/ยอดทั้งสองสกุลในชีตความเสียหาย"],
          ],
        },
        { t: "tip", text: "ถ้าวันไหนลืมลงเรต ระบบจะใช้เรตของวันล่าสุดก่อนหน้าไปก่อนและขึ้นข้อความเตือนไว้ พร้อมส่งแจ้งเตือนเข้าศูนย์การแจ้งเตือน" },
      ],
    },
    {
      id: "pm",
  ...
  function blockHtml(b) {
    switch (b.t) {
      ...
      case "h":
        return `<h4 class="mn-h4">${b.text}</h4>`;
      case "tip":
