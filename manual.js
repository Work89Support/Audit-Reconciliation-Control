...
const Manual = (() => {
  ...
  const IMG_SIZE = {"01-overview": [2360, 1600], "02-sidebar": [528, 1600], "03-topbar": [1728, 220], "04-filters": [1728, 428], "05-import-before": [2360, 1600], "06-import-after": [2360, 1600], "07-parse-result": [1728, 1852], "08-engine-validation": [1728, 936], "09-dashboard": [2360, 1600], "10-kpi-tiles": [1728, 538], "11-hourly-chart": [1728, 1020], "12-intake": [2360, 1600], "13-exceptions": [2360, 1600], "14-drawer": [1120, 1600], "15-drawer-checklist": [1120, 1600], "16-matching": [2360, 1600], "17-approvals": [2360, 1600], "18-damage": [2360, 1600], "19-pm": [2360, 1600], "20-kpi": [2360, 1600], "21-reports": [2360, 1600], "22-export-dialog": [1120, 1344], "23-panel-camera": [476, 42], "24-talk": [2360, 1600], "25-rules": [2360, 1600], "26-users": [2360, 1600], "27-notifications": [2360, 1600], "28-schedule": [2360, 1600], "29-auditlog": [2360, 1600], "30-roadmap": [2360, 1600], "31-fx": [2360, 1600], "32-clarify": [2360, 1600]};
  ...
  const SECTIONS = [
    ...
    },
    {
      id: "banks",
      group: "งานประจำวัน",
      title: "ธนาคารที่รองรับ และรูปแบบไฟล์",
      blocks: [
        { t: "p", text: "ระบบอ่าน statement ธนาคารที่เป็นไฟล์ <b>PDF ข้อความจริง</b> (ไม่ใช่ภาพสแกน) โดยตรวจเองจากหัวกระดาษว่าเป็นธนาคารไหน ไม่ต้องเลือกชนิดไฟล์ ตอนนี้รองรับ 7 ธนาคารที่ใช้งานจริง แต่ละธนาคารวางข้อมูลคนละแบบ ระบบจึงมีตัวอ่านเฉพาะของแต่ละราย" },
        {
          t: "table",
          head: ["ธนาคาร", "ช่องทาง", "รูปแบบปี", "เวลา", "จุดสังเกตของไฟล์"],
          rows: [
            ["KBANK", "K PLUS", "ค.ศ. ย่อ (26)", "มี HH:MM", "หัวมี “เลขที่บัญชีเงินฝาก” · ช่องทาง K PLUS"],
            ["LBK (LINE BK)", "LINE BK", "ค.ศ. ย่อ (26)", "มี HH:MM", "ฟอร์แมตเดียวกับกสิกร (เคลียริงเดียวกัน) ต่างที่ช่องทางเป็น LINE BK"],
            ["SCB", "ENET", "ค.ศ. ย่อ (26)", "มี HH:MM", "มี marker X1/X2/XB · คำอธิบายอยู่บรรทัดเหนือรายการ"],
            ["KTB", "MOBILE", "พ.ศ. ย่อ (69)", "มี / ไม่มี", "วันที่กับเวลาอยู่คนละบรรทัด · ฝาก = เงินโอนเข้า, ถอน = โอนเงินออก"],
            ["BBL", "mPhone", "ค.ศ. ย่อ (26)", "ไม่มี", "ไม่มีคอลัมน์เวลา · เข้า = TRF FR OTH BK, ออก = TRF TO OTH BK"],
            ["BAY", "MOBILE", "ค.ศ. เต็ม (2026)", "มี (วินาที)", "เวลามีวินาที · บรรทัด “บัญชีปลายทาง” อยู่ถัดลงมา"],
            ["TMN", "MOBILE", "—", "มี", "เลขบัญชีเป็นเบอร์มือถือ · คอลัมน์ เงินเข้า / เงินออก / ยอดคงเหลือ"],
          ],
        },
        { t: "h", text: "ตัวอย่างบรรทัดจริงของแต่ละธนาคาร" },
        { t: "p", text: "บรรทัดตัวอย่างด้านล่างคือรูปแบบที่ระบบคาดหวัง ใช้เทียบดูว่าไฟล์ที่ได้มาหน้าตาตรงไหม (ปิดชื่อผู้รับไว้)" },
        {
          t: "ul",
          items: [
            "<b>KBANK</b> — <code>19-07-26 00:00 รับโอนเงิน 100.00 1,035.25 K PLUS จาก …</code>",
            "<b>LBK</b> — <code>09-08-26 07:06 โอนเงิน 211.00 7,867.04 LINE BK โอนไป SCB X2448 …</code>",
            "<b>SCB</b> — คำอธิบายอยู่บรรทัดเหนือ แล้วตามด้วย <code>18/07/26 07:04 X2 ENET 5,000.00 23,932.00</code>",
            "<b>KTB</b> — บรรทัดบน <code>29/06/69 เงินโอนเข้า (IORSDT) 014-6444474223 30.00 16,492.01 606</code> · เวลาอยู่บรรทัดล่าง <code>22:55</code>",
            "<b>BBL</b> — <code>10/06/26 TRF FR OTH BK 14.00 1,313.58 mPhone</code> (ไม่มีเวลาในบรรทัด)",
            "<b>BAY</b> — <code>19/06/2026 22:28:12 โอนเงิน 144.00 1,278.86 MOBILE SCB …</code> · บรรทัดถัดมา <code>บัญชีปลายทาง : X532003</code>",
            "<b>TMN</b> — คอลัมน์ เงินเข้า/เงินออก/ยอดคงเหลือ · เลขบัญชีเป็นเบอร์มือถือ เช่น <code>081-2792-075</code>",
          ],
        },
        { t: "tip", text: "KTB บางบัญชีไม่แสดงเวลา และ BBL ไม่มีเวลาเลย — กรณีไม่มีเวลา ระบบจะจับคู่ด้วย บัญชี + ยอด + ทิศทาง ภายในวันแทน (ไม่ตัดรายการทิ้ง)" },
        { t: "warn", text: "ถ้าไฟล์เป็น PDF ที่สแกนมาเป็นภาพ (ไม่มีข้อความจริง) ระบบจะอ่านไม่ออก ต้องขอไฟล์ที่ export เป็นข้อความจากแอปธนาคารโดยตรง" },
      ],
    },
    {
      id: "auto",
    ...
    },
    {
      id: "clarify",
      group: "ตรวจสอบ",
      title: "งานชี้แจง — 2 ระบบ XB และ 123",
      blocks: [
        { t: "img", src: IMG("32-clarify"), cap: "หน้างานชี้แจง แยกตามระบบต้นทาง พร้อมตารางกำหนดว่าบริษัทไหนอยู่ระบบไหน" },
        { t: "p", text: "แผนกตรวจอยู่ 2 ระบบ และแต่ละระบบมีจังหวะส่งชี้แจงไม่เหมือนกัน สิ่งที่กำหนดว่าเคสหนึ่งจะไปทางไหนคือ <b>บริษัทต้นทางอยู่ระบบไหน</b> ไม่ใช่ความรุนแรงของเคส" },
        {
          t: "table",
          head: ["ระบบ", "จังหวะส่ง", "ใครชี้แจง", "กำหนดส่งคืน"],
          rows: [
            ["<b>XB</b>", "ทุกวัน ภายใน 17:00", "หัวหน้ากะ", "ชี้แจงกลับภายใน 24 ชม."],
            ["<b>123</b>", "รวบเป็นรอบ 1-15 / 16-25 / 26-สิ้นเดือน", "ออดิท 1 / 2 / 3 ตีไฟล์ให้ชี้แจง", "2-3 วันหลังปิดรอบ"],
          ],
        },
        { t: "warn", text: "ระบบ XB ส่ง <b>ทุกเคส</b> ไม่ว่าจะ Critical หรือ Low — ไม่ได้คัดเฉพาะเคสร้ายแรง ส่วนระบบ 123 ไม่ต้องส่งรายวันเลย รอรวบเป็นรอบ" },
        { t: "h", text: "ตั้งค่าครั้งเดียว: บริษัทไหนอยู่ระบบไหน" },
        {
          t: "steps",
          items: [
            "เปิดเมนู “งานชี้แจง”",
            "ดูแผง “บริษัทไหนอยู่ระบบไหน” ด้านบน",
            "กดปุ่ม <b>XB · รายวัน</b> หรือ <b>123 · รายรอบ</b> ให้ตรงกับความจริงของแต่ละบริษัท",
            "ระบบจำไว้ให้ และจัดเคสใหม่ทันทีโดยไม่ต้องกระทบยอดใหม่",
          ],
        },
        { t: "p", text: "บริษัทใหม่ที่เพิ่งมีไฟล์เข้ามาครั้งแรกจะขึ้นแถบแดงว่ายังไม่ได้กำหนด ระบบจัดเป็นรายรอบไว้ก่อนเพื่อไม่ให้เคสหาย แล้วเตือนให้มาเลือก" },
        { t: "h", text: "การออกเอกสาร" },
        {
          t: "table",
          head: ["ระบบ", "ปุ่มที่ใช้", "ได้เอกสารอะไร"],
          rows: [
            ["XB", "การ์ดของแต่ละกะ → “ออกใบขอให้ชี้แจง”", "ใบขอให้ชี้แจงของกะนั้น รวมทุกเคสในกะ พร้อมช่องให้หัวหน้ากะเขียนตอบ"],
            ["123", "การ์ดของแต่ละรอบ → “ออกใบขอให้ชี้แจง”", "ใบขอให้ชี้แจงของรอบนั้น พร้อมกำหนดส่งคืนที่คำนวณจากวันปิดรอบ"],
            ["ทั้งสอง", "เปิดเคส → “เอกสารชี้แจง”", "เอกสารชี้แจงฉบับสมบูรณ์ของเคสเดียว มีตารางเทียบ STM กับ BO และช่องแนบหลักฐาน"],
          ],
        },
        { t: "tip", text: "ปรับเวลาตัดรอบ 17:00 และจำนวนวันให้ชี้แจงได้ที่หน้า “ตั้งค่าระบบ” → สายงานและกำหนดเวลาชี้แจง" },
      ],
    },
    {
      id: "approve",
