...
assert.equal(hasLiveCoreError('ผลกระทบยอด', ['ยอดเมลและไฟล์: timeout']), false);
assert.equal(hasLiveCoreError('ผลกระทบยอด', []), false);
assert.doesNotMatch(app, /coreErrors\.includes/);
assert.match(app, /ยอดเสี่ยงเฉพาะ .*ไม่ใช่ยอดรวมทั้งระบบ/);
assert.match(app, /!liveOverviewState.exceptionsReady \? "ยอดเสี่ยงยังโหลดไม่สำเร็จ"/);
...
assert.match(app, /await Sb\.confirmDamage\([\s\S]+await loadLiveDamage\(true\)/, "บันทึกความเสียหายต้องรอ RPC สำเร็จและรีเฟรชทะเบียน");
const damageHandler=app.slice(app.indexOf('$("#btnDamage").addEventListener'),app.indexOf('$("#btnApprove").addEventListener'));
assert.ok(damageHandler.indexOf('await Sb.confirmDamage(')<damageHandler.indexOf('e.status = "damage"'), "ห้ามเปลี่ยนสถานะก่อนฐานข้อมูลยืนยัน");
assert.ok(!damageHandler.includes('Sb.post("damages"'), "ห้ามกลับไปใช้การเขียนสองขั้นตอน");
