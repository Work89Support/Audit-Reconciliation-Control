...
setImmediate(()=>{assert.ok(container.innerHTML.includes('EX-1'));assert.ok(container.innerHTML.includes('355.00'));assert.ok(container.innerHTML.includes('สรุปยอดแยกทุกหน้า'));assert.ok(container.innerHTML.includes('ผลต่างหลังรวมข้ามวัน'));console.log('Audit live sheets: five-company read-only adapter, deduplicated totals, cross-day totals and login guard passed');});
