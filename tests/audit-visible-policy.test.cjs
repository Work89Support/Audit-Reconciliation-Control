...
assert.deepEqual(policy.filter(historical).map(row=>row.id),['missing','other-company']);
...
assert.deepEqual(view.rows.map(row=>row.id),['missing','other-company']);
assert.equal(view.counts.review,2);
console.log('Audit visible policy: five XB companies hide informational large-amount alerts only');
