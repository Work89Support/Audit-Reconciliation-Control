...
assert.ok(ctx.accountHtml.includes('<b>บัญชีลูกค้าใน BO</b>'));
assert.ok(ctx.accountHtml.includes('เลขบัญชี: 0012345678'));
assert.ok(ctx.accountHtml.includes('ชื่อลูกค้า: &lt;ชื่อ&gt;'));
assert.ok(ctx.accountHtml.includes('ท้าย 4 ตัว: <b>5678</b>'));
...
vm.runInContext(app.slice(customerEnd, app.indexOf('function groupReviewCases', customerEnd)), ctx);
ctx.example.customerDetails.bo.origin = 'เติมมือ';
ctx.example.customerDetails.bo.performedBy = 'Meta X8';
ctx.example.customerDetails.bo.note = '<script>note</script>\nบรรทัดสอง';
vm.runInContext('globalThis.manualHtml = reviewCustomerHtml(example, "bo")', ctx);
assert.ok(ctx.manualHtml.includes('ทำรายการโดย (N): <b>Meta X8'));
assert.ok(ctx.manualHtml.includes('หมายเหตุ (O): &lt;script&gt;note&lt;/script&gt;\nบรรทัดสอง'));
assert.ok(!ctx.manualHtml.includes('<script>'));
ctx.example.customerDetails.bo.origin = 'เติม ออโต้';
vm.runInContext('globalThis.autoHtml = reviewCustomerHtml(example, "bo")', ctx);
assert.ok(!ctx.autoHtml.includes('ทำรายการโดย (N)'));
