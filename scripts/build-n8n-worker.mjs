...
const boFirstCoverage=(()=>{
  ...
  const collect=records=>{const map=new Map();for(const r of records||[]){const x=ident(r);if(!x.id||x.id==='UNKNOWN')continue;const company=companyOf(r),system=systemOf(company),d=dir(r),key=system+'|'+company+'|'+x.kind+'|'+x.id+'|'+d,item=map.get(key)||{key,system,company,kind:x.kind,identity:x.id,label:x.label,direction:d,rows:0,amount:0,source_files:[]};item.rows++;item.amount=Math.round((item.amount+Number(r.amount||0))*100)/100;const sourceFile=String(r.source_file||r.fileName||'').trim();if(sourceFile&&!item.source_files.includes(sourceFile))item.source_files.push(sourceFile);map.set(key,item)}return[...map.values()].sort((a,b)=>a.key.localeCompare(b.key))};
...
return [{json:{job,result:{run_by:'n8n-cloud-worker',elapsed_ms:result.elapsedMs||Date.now()-started,stm_count:result.stmCount||0,bo_count:result.boCount||0,matched:result.matched||0,match_rate:Number((result.matchRate||0).toFixed(3)),no_stm_count:result.noStmCount||0,file_ids:fileIds,summary:{rules_only:!!result.rulesOnly,rule_exceptions:resolvedRuleExceptions.length,worker_version:'1.4.1',exact_unique_tolerance_sec:600,pm_master_account_guard:true,bo_first:boFirstCoverage}},exceptions,files:parseResults,quality_errors:[]},pairedItem:{item:0}}];`;
